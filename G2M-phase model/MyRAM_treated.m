function  [Para_col_accepted,  acc_rate_col,    S_curr, Para_col_all,Diff_norm_col]  = MyRAM_treated(  T, num_para, Para_col_accepted , S_curr,myfun,Var_name, outputfile_dir, Model_Index, num_HistPoints,lb,ub, ...
    para_change_exclude_idx,  para_fixed, outfile, sample, curr_T, Diff_norm_col, Para_col_all )
...
[~, Target_curr,  diff_norm_sum]   = myfun( theta0 , Model_Index ,outfile, model_obj, lb,ub, para_change_exclude_idx,  para_fixed  )  ;
...
while  curr_T- num_HistPoints+1 <= T 
    ...
    while ~ValidFlag
         ...
         while   any(next_para < 0)  ||  prior_val_prod  == 0 ||    ( sum(next_para(1:3))>1 &&   local_model_obj.TreatmentEffectOnPhases == 3 )  || ( sum(next_para([4,8]))>1 &&   local_model_obj.TreatmentEffectOnPhases == 3 )  ||   ( sum(next_para(2:3))>1 &&   local_model_obj.TreatmentEffectOnPhases == 2)   
          ...
          try 
                   [exception, Target_next,  diff_norm_sum]  = myfun( next_para, Model_Index, outfile ,local_model_obj , lb,ub, para_change_exclude_idx,  para_fixed)  ;
