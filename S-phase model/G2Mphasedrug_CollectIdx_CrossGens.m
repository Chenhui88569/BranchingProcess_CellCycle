function [a_G1, a_S, a_G2M]  = G2Mphasedrug_CollectIdx_CrossGens( AbsolutePhase, model_obj)
...
% Repeating states
for k = 2: model_obj.num_RroundsOfTreatedcells
    additional_states = { ...
        ['G1(', num2str(k+1), ')'], ...
        ['UR_G2M(', num2str(k), ')'], ...
        ['FR_G2M(', num2str(k), ')'], ...
...
if model_obj.num_GenOfHealthycells-1>0
    for k = 1:model_obj.num_GenOfHealthycells-1
        order = model_obj.num_RroundsOfTreatedcells+1+k;
        additional_states = { ...
            ['G1(', num2str( order ), ')'], ...
            ['S(', num2str( order  ), ')'] ...
            ['G2M(', num2str( order  ), ')'] ...
...
for i = 1:length(state_list)
    ...
    if contains(state, 'G1(')
        num = regexp(state, 'G1\((\d+)\)', 'tokens');
        num = str2double(num{1}{1});
        a_G1{num} = [a_G1{num}, i];
    elseif contains(state, 'S(')
         num = regexp(state, 'S\((\d+)\)', 'tokens');
         num = str2double(num{1}{1});
        a_S{num} = [a_S{num}, i];
    elseif contains(state, 'G2M(')
         num = regexp(state, 'G2M\((\d+)\)', 'tokens');
         num = str2double(num{1}{1});
