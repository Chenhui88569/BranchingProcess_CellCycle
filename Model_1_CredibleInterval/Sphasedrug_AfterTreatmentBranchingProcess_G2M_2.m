function mat_M = Sphasedrug_AfterTreatmentBranchingProcess_G2M_2( G1_block,   mat_m , S_arrest_UR,  S_arrest_FR,  tilde_G2M, G2M_2, G1_cdf, S_cdf, G2M_cdf,  ...
...
mat_G1 = mat_m;
[size1,~] = size(mat_G1);
mat_m = zeros( size1+1,  size1+1 );
mat_m(1,2) = 2;
mat_m(2:end, 2:end) =  mat_G1;
...
mat_G(4,4,: ) = S_cdf;
mat_G(5,5,: ) = G2M_cdf;
mat_G(6,6,: ) = S_arrest_UR;
mat_G(7,7,: ) =  S_arrest_FR;
mat_G(8,8,: ) = tilde_G2M;
%% Repeated block
for i = 1: num_RroundsOfTreatedcells-1
    startnum = 9 +( i-1 )*7;
    endnum =  9 + i*7-1;
    mat_G(startnum   : endnum, startnum :  endnum,:    ) = mat_G( 2 :8, 2:8 ,:);
end

% mat_G(9,9,: ) = G1_cdf;
% mat_G(10,10,: ) = G1_block ;
% mat_G(11,11,: ) = S_cdf;
% mat_G(12,12,: ) = G2M_cdf;
% mat_G(13,13,: ) = S_arrest_UR;
% mat_G(14,14,: ) =  S_arrest_FR;
% mat_G(15,15,: ) = tilde_G2M;
if True_num_GenOfHealthycells > 0
    for i = 1:True_num_GenOfHealthycells
        startnum = 7*num_RroundsOfTreatedcells + 2 + (i-1)*3;
