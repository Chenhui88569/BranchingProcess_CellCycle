function mat_M = G2Mphasedrug_AfterTreatmentBranchingProcess_S_2( mat_m ,G2_arrest_UR,  G2_arrest_FR,  G2_arrest_MS,   tilde_G1,  tilde_S, S_2 , G1_cdf, S_cdf, G2M_cdf, ...
...
mat_G(2,2,: ) = G2M_cdf;
mat_G(3,3, : ) = G1_cdf;
mat_G(4,4,: ) = G2_arrest_UR;
mat_G(5,5,: ) =  G2_arrest_FR;
mat_G(6,6,: ) = G2_arrest_MS;
mat_G(7,7,: ) = tilde_G1;
mat_G(8,8,: ) = tilde_S;
mat_G(9,9, :) = S_cdf;
mat_G(10,10, :) = G2M_cdf;
...
for i = 1: num_RroundsOfTreatedcells-1
    startnum = 11 +( i-1 )*8;
    endnum = 11 + i*8-1;
    mat_G(startnum   : endnum, startnum :  endnum,:    ) = mat_G( 3:10, 3:10,:);
end
newG2MWithoutdrug = 1 + 8*num_RroundsOfTreatedcells  +1;
