    ...
    methods
        ...
        function obj = create_model(Model_Index, para_unknown, output_dir)
            ...
            elseif  obj.data_time(end) > 48
                    obj.num_RroundsOfTreatedcells= 5;   %5;
                    obj.num_GenOfHealthycells =  1;
            end
            obj.dist_save_flag  = 0;
        end
        ...
        function  [mat_M_convergence_BeforeTreatment_complete,   mat_M_G1_BeforeTreatment, States_convergence,   M_aftertreatment_col] = M_aftertreatment(obj)
            ...
            if obj.TreatmentEffectOnPhases == 3 % G2M phase cells
                    ...
                    if obj.dist_save_flag ==1 
                        fprintf('UR scale: %f , shape %f\n',lambda_G2Marrest_UR, tilde_b);
                        fprintf('FR scale: %f , shape  %f\n',lambda_G2Marrest_FR, tilde_b);
                        fprintf('MS scale: %f , shape  %f \n',lambda_G2Marrest_MS, tilde_b);
                        
                        num_sample = 10000;
                        tilde_G1_RandomSamples = generateRandomNumbersAndHistogram(obj,tilde_G1,num_sample);
                        tilde_S_RandomSamples = generateRandomNumbersAndHistogram(obj,tilde_S,num_sample);
                        writematrix(    tilde_G1_RandomSamples(:)   , sprintf('model%d_tileG1.csv', obj.Model_Index) ); 
                        writematrix(  tilde_S_RandomSamples(:)  , sprintf('model%d_tileS.csv', obj.Model_Index) );
            ...
            if obj.TreatmentEffectOnPhases == 2 % S phase cells
                ...
                if obj.dist_save_flag ==1
                    fprintf('UR scale: %f , shape %f\n',lambda_Sarrest_UR, tilde_b);
                    fprintf('FR scale: %f , shape  %f\n',lambda_Sarrest_FR, tilde_b);
                    fprintf('G1 block: %f , shape  %f\n',lambda_G1block, tilde_b);
                   
                    num_sample = 10000;
                    tilde_G2M_RandomSamples = generateRandomNumbersAndHistogram(obj,tilde_G2M,num_sample);
                    writematrix(      tilde_G2M_RandomSamples(:)   , sprintf('model%d_tileG2M.csv', obj.Model_Index) );
                    
                end
                num_CyclingState = size(obj.States_convergence_RemainingTimeDist,1);
                States_convergence = States_convergence(1: num_CyclingState );
                parpool(5)
