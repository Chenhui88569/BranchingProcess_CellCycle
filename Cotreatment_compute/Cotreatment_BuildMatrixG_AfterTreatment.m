function   mat_G = Cotreatment_BuildMatrixG_AfterTreatment(obj,G1_pdf, S_pdf, G2M_pdf, G1_cdf,G2M_cdf,S_cdf,TotalState)
...
%% G2M phase drug
% create G2 arrest
lambda_fun = @(lambda_max) lambda_max * ((Dose_G2M / EC50_G2M)^n_G2M) / (1 + (Dose_G2M/ EC50_G2M)^n_G2M);
...
lambda_d_G1_G2M =  lambda_fun(lambda_d_max_G1_G2M);
...
[ m_Dd , Dd_pdf , ~ ]  =   PdfcdfCal_treated('Weibull',  obj.t_span ,  lambda_d_G1_G2M  , tilde_b_d_G2M  );
Dd_pdf( Dd_pdf == inf) = 0;
...
lambda_d_S_G2M =  lambda_fun(lambda_d_max_S_G2M);
...
[ m_Dd , Dd_pdf , ~ ]  =   PdfcdfCal_treated('Weibull',  obj.t_span ,  lambda_d_S_G2M  , tilde_b_d_G2M  );
Dd_pdf( Dd_pdf == inf) = 0;
...
%% S phase drug
% create S arrest
lambda_fun = @(lambda_max) lambda_max * ((Dose_S  / EC50_S)^n_S) / (1 + (Dose_S  / EC50_S)^n_S);

% create G1 block
lambda_G1block  = lambda_fun(lambda_G1block_max );
[~, ~, G1block ]  =   PdfcdfCal_treated('Weibull',  obj.t_span,  lambda_G1block  , tilde_b_S  ); %A: scale, B , shape

% create S arrest
lambda_Sarrest_UR  = lambda_fun(lambda_Sarrest_UR_max);
[~, ~, S_arrest_UR ]  =   PdfcdfCal_treated('Weibull',  obj.t_span , lambda_Sarrest_UR  , tilde_b_S  ); %A: scale, B , shape
lambda_Sarrest_FR  = lambda_fun(lambda_Sarrest_FR_max);
[~, ~, S_arrest_FR ]  =   PdfcdfCal_treated('Weibull',  obj.t_span , lambda_Sarrest_FR  , tilde_b_S  ); %A: scale, B , shape
% Create tilde_G2M
lambda_G2Marrest = lambda_fun(lambda_G2Marrest_max);
lambda_d_G2M =  lambda_fun(lambda_d_max_G2M);
[ m_D_G2Marrest ,D_G2Marrest_pdf , ~ ]  =   PdfcdfCal_treated('Weibull',  obj.t_span ,   lambda_G2Marrest   , tilde_b_S  );
D_G2Marrest_pdf(D_G2Marrest_pdf  == inf) = 0;
[ m_Dd , Dd_pdf , ~ ]  =   PdfcdfCal_treated('Weibull',  obj.t_span ,  lambda_d_G2M  , tilde_b_d_S  );
Dd_pdf( Dd_pdf == inf) = 0;
ChoiceOfDist_treated_U = 'Exponential';
[m_U, U_pdf, ~]  =   PdfcdfCal_treated(ChoiceOfDist_treated_U, obj.t_span, 1 ,[] );
U_pdf( U_pdf == inf) = 0;
...
for i = 1: obj.num_RroundsOfTreatedcells
    startnum = 20 +( i-1 )*10;
    endnum =  20 + i*10 -1;
    mat_G(startnum   : endnum, startnum :  endnum,:    ) = mat_G( 10 :19, 10:19,:);
end
...
[~, ~, G_apop_cdf]  =   PdfcdfCal_treated('Exponential',  obj.t_span_AfterTreatment  ,5 ,  [] );
